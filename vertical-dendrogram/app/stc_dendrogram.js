stoicReady(1, function() {
  ...
  define('stc_dendrogram', ['d3', 'utils'], function(d3, utils) {
    ...
    perspective.bind = function ($scope, $container, demoData) {
      ...
      try {
        ...
        var vis = d3content.append("svg:svg")
            ...
            .attr("height", '100%')
            .append("svg:g")
              .attr("transform", "translate(" + 10 + "," + top + ")");
        ...
        var moveSvg = function(callback) {
            ...
            var newHeight = top+ (maxY+1)*space;
            if(newHeight >= _h) {
              $content.stop();
              $content.animate({'height':newHeight}, duration);
              if(newHeight>oldHeight) {$container.animate({'scrollDown':newHeight}, duration);}
            } else if(newHeight < _h) {
              $content.animate({'height':_h}, duration);
            }

            var newPos = (newHeight > _h/2 && maxY>1) ? minY*space+top : top;
            ...
            if(newPos !== trans.y) {
              d3g
                .transition()
                .duration(duration)
                .attr('transform', 'translate('+trans.x+','+newPos+')')
                .each('end', callback)
                ;
            } else if(callback) {
              callback();
        ...
        var nextUp, nextDown, nextPlace, currentRow = 0;
        var dragAndDrop = function(d, x, y) {
          var newNextUp, newNextDown, row, diff, oldDiffD, oldDiffU;
          ...
          try {
            ...
            nodeEnter.append("svg:text")
                ...
                .attr("transform",  function(d) {
                  return (d.hasChildren ? "rotate(0)" : "rotate(30)") ;
                })
        ...
        nodeCentral = vis.append("svg:g")
            ...
            .attr("transform", function() { return "translate(" + (_w/2) + "," + 0 + ")"; })
            .on("click", function(d) {
              if(canClick) {
                if(elementSelected) {elementSelected.classed("highlighted", false);}
                elementSelected = d3.select(this);
        ...
        var root = {id: rootId, name: rootName, children:[], hasChildren:true, x0:_w/2, y0:0};
